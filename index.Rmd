...
in SQl you can filter colums by using SELECT and row by using WHERE. Let's look at an example using the Lahman baseball database. First, we should load the Laham package. we will also need the sqldf package.
...
Let's take a look at an example. suppose we want to see the homerun totals for the 1927 Yankees.
```{r}
